...
p.coded = 'No'; % change later to coded & uncoded
        ...
        for indx_subb = 1:p.nFreq(2)
            if (p.Fc + p.BW/2) < 0.55e12
                if p.tau_LoS > 1/p.clusterArrivalRateVec(indx_subb)
                    error('Paramters aren''t consistent: delay of LoS should be less than the delay of First Cluster');
                end
            end
        end
    end
...
%% Phase Noise parameters (Need to be updated !!)

if isfield(q,'AddTHzCH')
    p.AddTHzCH = q.AddTHzCH;
else
    p.AddTHzCH = 'Off';  % 'Off', default 'On'
end

if isfield(q,'AddAWGN')
    p.AddAWGN = q.AddAWGN;
else
    p.AddAWGN = 'On';  % 'Off', default 'On'
end

if isfield(q,'AddPHN')
    p.AddPHN = q.AddPHN;
else
    p.AddPHN = 'None';  %'Tx', 'Rx', 'Both' 'None'
end

if isfield(q,'PHN_Model')
    p.PHN_Model = q.PHN_Model;
else
    p.PHN_Model = 'Gaussian';  % % 'Wiener+Gaussian', 'Wiener', 'Gaussian', 'PSD-SSB'
end
...
% Need to be updated !!!!!!!!!!!!!!!!!!!
switch p.PHN_Model
    case 'Wiener+Gaussian'
        if isfield(q,'K0')
            p.K0 = q.K0;
        else
            p.K0 = 10^(-110/10);% -110 dBc/Hz (1 GHz)
        end
        if isfield(q,'Q0')
            p.Q0 = q.Q0;
        else
            p.Q0 = -90;% -90 dBc/Hz @ 100 KHz offset
        end        
        if isfield(q,'fgamma')
            p.fgamma = q.fgamma;
        else
            p.fgamma = 100e3; % Corner Frequency fc^2=K2/K0
        end                      
        
        
        p.K2 = p.fgamma^2*10^(p.Q0/10);
        p.fcor = sqrt(p.K2/p.K0);
        p.Sigma2_G = p.K0*p.BW; %/T here T=1/BW assume BW=Fc
        p.Sigma2_W = 4*pi^2*p.K2/p.BW; %*T here T=1/BW assume BW=Fc
    case 'Wiener'
    ...
    case 'PSD-SSB'
        ...
        if isfield(q,'phase_noise_freq')
            p.phase_noise_freq = q.phase_noise_freq;
        else
            p.phase_noise_freq = [ 4e3, 1e4, 1e5, 1e6, 1e7, 1e8, 4.99e8 ]; % Offset From Carrier
        end
        if isfield(q,'phase_noise_power')
            p.phase_noise_power = q.phase_noise_power;
        else
            p.phase_noise_power = [ -40, -70, -90, -105, -106 ,-107 ,-109 ]; % Phase Noise power
        end        
        
    otherwise
        error('This model isn''t implemented yet!');
end

%% CFO and STO

% Async.Type: three methods are implemented: CFO, STO, Both
% Async.eps for CFO is the relative frequency offset to inter-carrier separartion
if isfield(q,'Async')
    p.Async.Type = q.Async.Type;
    p.Async.eps = q.Async.eps;
else
    p.Async.Type = 'CFO'; % 'STO', 'Both
    p.Async.eps = 0;
end
